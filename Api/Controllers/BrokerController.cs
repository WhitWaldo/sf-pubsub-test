...
namespace Api.Controllers
{
    ...
    public class BrokerController : ControllerBase
    {
        ...
        [HttpGet("stats")]
        public async Task<ActionResult<Dictionary<string, IEnumerable<QueueStats>>>> Get(
            [FromQuery] BrokerStatsQueryParams queryParams)
        {
            try
            {
        ...
        public async Task<IActionResult> Publish([FromBody] object messageBody)
        {
            try
            {
                ...
                await _brokerClient.PublishMessageAsync(JsonSerializer.Deserialize(json, type));
            }
            catch (Exception e)
            {
                return BadRequest(e.Message);
